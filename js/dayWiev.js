...
function domJob(broj, mancha, time) {
  let newLi, spanBroj, spanMancha, spanTime, breakline;
  var spanMT,
    manchaTura = 0;
...
function show() {
  if (retrivedItem) {
    // we check if its array ol single input in witch case we dont use for each
    if (Array.isArray(retrivedItem.tura)) {
      retrivedItem.tura.forEach((ele) => {
        domJob(
          retrivedItem.tura[ele - 1],
          retrivedItem.mancha[ele - 1],
          retrivedItem.time[ele - 1]
        );
      });
      // if it's not an array
    } else {
      domJob(retrivedItem.tura, retrivedItem.mancha, retrivedItem.time);
    }
  }
}
